                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table text-md-nowrap" id="example1">
                            ...
                            <tbody>
                            @foreach($laboratory as $sublaboratory)
                                <tr>
                                    <td>{{ $loop->iteration}}</td>
                                    <td>{{ $sublaboratory->created_at }}</td>
                                    <td><a href="{{ route('doctor_invoices.show',$sublaboratory->id) }}">{{ $sublaboratory->Patient->name }}</a></td>
                                    <td>{{ $sublaboratory->doctor->name }}</td>
                                    <td>{{ $sublaboratory->description }}</td>
                                    <td>
                                        @if($sublaboratory->case == 0)
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu tx-13">
                                                <a class="dropdown-item" href="{{ route('LaboratoryEmployee.edit',$sublaboratory->id) }}"><i class="text-primary fa fa-stethoscope"></i>&nbsp;&nbsp;{{ __('doctor/invoices.Add Diagnostic') }}</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
